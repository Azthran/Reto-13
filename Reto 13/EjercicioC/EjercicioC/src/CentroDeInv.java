...
public class CentroDeInv {
        public static Map<String, Responsable> listaEncargados = new HashMap<>();
    ...
    private static void actualizarListaDeEncargados(String ci, int ct, long mu) {
        ...
        if(listaEncargados.containsKey(ci)) {
            Responsable prof = listaEncargados.get(ci);
            Responsable nuevoProf = new Responsable(ci, prof.totalEquipos += ct, prof.montoTotal += mu);
            listaEncargados.put(ci, nuevoProf);


        } else {
            Responsable ProfRegistrar = new Responsable(ci, (long) ct, mu);
